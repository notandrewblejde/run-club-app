...
export default function EditClubPostScreen() {
  ...
  useEffect(() => {
    ...
    setContent(postQ.data.content ?? '');
    setPhotos(postQ.data.photos ?? []);
  }, [postQ.data]);
  ...
  const submit = async () => {
    ...
    try {
      await updatePost.mutateAsync({
        postId,
        body: { content: content.trim(), photos },
      });
      router.back();
    } catch (e: unknown) {
      Alert.alert('Could not update post', (e as Error)?.message ?? 'Try again later');
    }
  };
  ...
  useEffect(() => {
    setActions([
      ...
      },
      {
        ...
        disabled: !content.trim() || updatePost.isPending || uploading || postQ.isLoading,
      },
    ]);
    return () => clearActions();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [content, photos, uploading, updatePost.isPending, deletePost.isPending, postQ.isLoading]);
    ...
    <View style={styles.container}>
      ...
      {postQ.isLoading ? (
        ...
        <ScrollView contentContainerStyle={styles.body} keyboardShouldPersistTaps="handled">
          ...
          <TouchableOpacity style={styles.deleteInline} onPress={confirmDelete} activeOpacity={0.85}>
            <Trash2 size={14} color={tokens.error} />
            <Text style={[styles.deleteInlineText, { color: tokens.error }]}>Delete post</Text>
          </TouchableOpacity>
        </ScrollView>
      )}
    </View>
...
function makeStyles(t: ThemeTokens) {
  return StyleSheet.create({
    ...
    deleteInlineText: { fontWeight: '600' },
  });
}
